...
public class UpnpUtil {
    ...
    public static UDN uniqueSystemIdentifier(String salt,String model,String manufacturer,String hostName,String hostAddress) {
        StringBuilder systemSalt = new StringBuilder();
        System.out.println(TAG+ "host:" + hostName + " ip:" +   hostAddress);
        if (null != hostName
                && null != hostAddress) {
            systemSalt.append(hostName).append(
                    hostAddress);
        }
        systemSalt.append(model);
        systemSalt.append(manufacturer);
                ...
                for (Enumeration<InetAddress> enumIpAddr = intf.getInetAddresses(); enumIpAddr
                    ...
                    if (!inetAddress.isLoopbackAddress()) {
                        ...
                        if (!ipaddress.contains("::")) {// ipV6的地址
                            System.out.println(TAG+ ipaddress);
                            return ipaddress;
                        }
                    }
                }
